...
import riskMatrixMuni.restapi.Services.Cat2SeverityService;

@RestController
@CrossOrigin(origins = "*", methods= {RequestMethod.GET,RequestMethod.POST})
@RequestMapping("/Severity")

public class Cat2SeverityController {

	@Autowired
	Cat2SeverityService entityService;
	
	@PostMapping("/severityDML")
	@Transactional
	List<Cat2Severity> getClasification(@RequestBody Cat2Severity pEntity){
		List<Cat2Severity> lstEntityRet = new ArrayList<Cat2Severity>();
		Cat2Severity entityInterno = new Cat2Severity();
		...
		if(pEntity.get_opcionDML()== 1 || pEntity.get_opcionDML() == 3 ) {
			...
			List<Cat2Severity> lstEntityB =  entityService.buscarxnombre(
					pEntity.getName()
					,pEntity.getMuni_id()
					);
