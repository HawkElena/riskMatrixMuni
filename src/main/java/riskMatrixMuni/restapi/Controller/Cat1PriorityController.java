...
import riskMatrixMuni.restapi.Services.Cat1PriorityService;

@RestController
@CrossOrigin(origins = "*", methods= {RequestMethod.GET,RequestMethod.POST})
@RequestMapping("/Priority")

public class Cat1PriorityController {

	@Autowired
	Cat1PriorityService entityService;

	@PostMapping("/priorityDML")
	@Transactional
	List<Cat1Priority> getPriority(@RequestBody Cat1Priority pEntity){
		List<Cat1Priority> lstEntityRet = new ArrayList<Cat1Priority>();
		Cat1Priority entityInterno = new Cat1Priority();
		...
		if(pEntity.get_opcionDML()== 1 || pEntity.get_opcionDML() == 3 ) {
			...
			if(pEntity.getTabla_id() < 1   ) {
				pEntity.setTabla_id(0);
			}
			
			List<Cat1Priority> lstEntityB =  entityService.buscarxnombre(
					pEntity.getName()
					,pEntity.getMuni_id()
					,pEntity.getTabla_id()
					);
